...
pub struct Symbol<T: PartialEq> (String, Option<Box<dyn Fn(&T)->bool>>);
...
impl <T:PartialEq> Symbol<T> {
    ...
    pub fn terminal(&self) -> Option<(&str, &dyn Fn(&T) -> bool)> {
...
impl <T:PartialEq> hash::Hash for Symbol<T> {
...
impl <T:PartialEq> fmt::Debug for Symbol<T> {
...
pub struct Rule<T :PartialEq + std::hash::Hash> {
    pub head: String,
    pub spec: Vec<Rc<Symbol<T>>>,
}

impl <T:PartialEq+std::hash::Hash> string::ToString for Rule<T> {
...
impl <T:PartialEq+std::hash::Hash> fmt::Debug for Rule<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.to_string())
    }
}

impl <T:PartialEq+std::hash::Hash> Rule<T> {
    #[cfg(test)]
...
#[derive(Clone,Debug)]
pub struct Grammar<T:PartialEq+std::hash::Hash> {
    pub start: String,
    pub rules: Vec<Rc<Rule<T>>>,
}


/// Builds a Gramar while validating existence of Symbols and checking rules.
pub struct GrammarBuilder<T:PartialEq + std::hash::Hash> {
    symbols: HashMap<String, Rc<Symbol<T>>>,
    rules: Vec<Rc<Rule<T>>>,
    error: Option<String>,
}

impl <T:PartialEq+std::hash::Hash> Default for  GrammarBuilder <T> {
    fn default() -> Self { 
        GrammarBuilder {
            symbols: HashMap::new(),
            rules: Vec::new(),
...
impl <T:PartialEq+std::hash::Hash> GrammarBuilder <T> {
    fn _add_symbol(&mut self, symbol: Symbol<T>, quiet: bool) {
    ...
    pub fn into_grammar<S>(mut self, start: S) -> Result<Grammar<T>, String>
        where S: Into<String>
    {
