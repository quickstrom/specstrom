...
type LeafBuilder<'a, ASTNode, T> = Box<dyn Fn(&str, &T) -> ASTNode + 'a>;

pub struct EarleyForest<'a, ASTNode: Clone, T:PartialEq> {
    actions: HashMap<String, SemAction<'a, ASTNode>>,
    leaf_builder: LeafBuilder<'a, ASTNode, T>,
}

impl<'a, ASTNode: Clone, T:PartialEq> EarleyForest<'a, ASTNode, T> {
    pub fn new<Builder>(leaf_builder: Builder) -> Self
            where Builder: Fn(&str, &T) -> ASTNode + 'a {
        EarleyForest{
            actions: HashMap::new(),
...
impl<'a, ASTNode: Clone, T:PartialEq + std::hash::Hash + Eq+Clone> EarleyForest<'a, ASTNode, T> {
    fn reduce(&self, root: &Rc<Item<T>>, args: Vec<ASTNode>)
...
impl<'a, ASTNode: Clone, T:PartialEq+std::hash::Hash+Eq+Clone> EarleyForest<'a, ASTNode, T> {
    ...
    fn walker(&self, root: &Rc<Item<T>>) -> Result<Vec<ASTNode>, String> {
    ...
    pub fn eval(&self, ptrees: &ParseTrees<T>) -> Result<ASTNode, String> {
...
impl<'a, ASTNode: Clone, T:PartialEq+std::hash::Hash+Eq+Clone> EarleyForest<'a, ASTNode, T> {
    ...
    fn walker_all(&self, root: &Rc<Item<T>>) -> Result<Vec<Vec<ASTNode>>, String> {
    ...
    pub fn eval_all(&self, ptrees: &ParseTrees<T>) -> Result<Vec<ASTNode>, String> {
        let mut trees = Vec::new();
