...
pub enum BackPointer<T:PartialEq+std::hash::Hash+Clone> {
    Complete(Rc<Item<T>>, Rc<Item<T>>),
    Scan(Rc<Item<T>>, T),
}

/// An Item is a partially matched `Rule`. `dot` shows the match progress.
pub struct Item<T:PartialEq+std::hash::Hash+Clone> {
    pub rule: Rc<Rule<T>>,  // LR0item (dotted rule)
    ...
    backpointers: cell::RefCell<HashSet<BackPointer<T>>>,
}
...
impl <T:PartialEq+std::hash::Hash+Clone> hash::Hash for Item<T> {
...
impl <T:PartialEq+std::hash::Hash+Clone> PartialEq for Item<T> {
...
impl <T:PartialEq+std::hash::Hash+Clone> Eq for Item<T> {}

impl <T:PartialEq+std::hash::Hash+Clone> fmt::Debug for Item<T> {
...
impl <T:PartialEq+std::hash::Hash+Eq+Clone> Item<T> {
    ...
    pub fn sources(&self) -> cell::Ref<HashSet<BackPointer<T>>> {
        self.backpointers.borrow()
    }

    /// Merge other Item into this one moving over its backpointers
    pub fn merge_sources(&self, other: Item<T>) {
    ...
    pub fn predict_new(rule: &Rc<Rule<T>>, start: usize) -> Item<T> {
        Item{
            rule: rule.clone(),
            dot: 0,
            start,
    ...
    pub fn scan_new(source: &Rc<Item<T>>, end: usize, input: &T) -> Item<T> {
        let mut _bp = HashSet::new();
        _bp.insert(BackPointer::Scan(source.clone(), input.clone()));
        Item{
    ...
    pub fn complete_new(source: &Rc<Item<T>>, trigger: &Rc<Item<T>>, end: usize) -> Item<T> {
        let mut _bp = HashSet::new();
...
mod tests {
    ...
    fn item(rule: Rc<Rule<str>>, dot: usize, start: usize, end: usize) -> Item<str> {
